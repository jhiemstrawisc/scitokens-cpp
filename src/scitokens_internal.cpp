...
std::string es256_from_coords(const std::string &x_str,
  ...
  if (!EC_POINT_set_affine_coordinates(ec_group.get(), Q_point.get(),
                                       x_bignum.get(), y_bignum.get(), NULL)) {
    throw UnsupportedKeyException("Invalid elliptic curve point in key");
  }

  size_t out_len = EC_POINT_point2buf(
      ec_group.get(), Q_point.get(), POINT_CONVERSION_UNCOMPRESSED, &buf, NULL);
  if (out_len == 0) {
  ...
  if (!param_build.get() ||
      !OSSL_PARAM_BLD_push_utf8_string(param_build.get(), "group", "prime256v1",
                                       0) ||
  ...
  if (EC_POINT_set_affine_coordinates_GFp(params, Q_point.get(), x_bignum.get(),
                                          y_bignum.get(), NULL) != 1) {
    throw UnsupportedKeyException("Invalid elliptic curve point in key");
  }
...
void Validator::get_public_keys_from_web(const std::string &issuer,
                                         unsigned timeout,
                                         picojson::value &keys,
                                         int64_t &next_update,
                                         int64_t &expires) {
  ...
  if (status_code != 200) {
    ...
    if (status_code != 200) {
      throw CurlException(
          "Failed to retrieve metadata provider information for issuer.");
    }
  }
  char *buffer;
  size_t len;
...
bool Validator::refresh_jwks(const std::string &issuer) {
  ...
  picojson::value keys;
  get_public_keys_from_web(issuer, SimpleCurlGet::default_timeout, keys,
                           next_update, expires);
...
void Validator::get_public_key_pem(const std::string &issuer,
                                   const std::string &kid,
  ...
  if (get_public_keys_from_db(issuer, now, keys, next_update)) {
    ...
    get_public_keys_from_web(issuer, SimpleCurlGet::extended_timeout, keys,
                             next_update, expires);
    store_public_keys(issuer, keys, next_update, expires);
  }
...
bool scitokens::Enforcer::scope_validator(const jwt::claim &claim,
  ...
  while (scope_iter != scope.end()) {
    ...
    // If we are in compatibility mode and this is a WLCG token, then translate
    // the authorization names to utilize the SciToken-style names.
