...
SimpleCurlGet::GetStatus SimpleCurlGet::perform_continue() {
  ...
  if (!resm && still_running) {
    ...
    resm = curl_multi_fdset(m_curl_multi.get(), m_read_fd_set, m_write_fd_set,
                            m_exc_fd_set, &m_max_fd);
    if (resm) {
      throw CurlException(curl_multi_strerror(resm));
    }
    if (m_max_fd < 0)
...
SciToken::deserialize_continue(std::unique_ptr<SciTokenAsyncStatus> status) {
  ...
  if (status->m_status) {
    ...
    status->m_status =
        status->m_validator->verify_async_continue(std::move(status->m_status));
  }
...
Validator::get_public_key_pem_continue(std::unique_ptr<AsyncStatus> status,
  ...
  if (status->m_do_store) {
    store_public_keys(status->m_issuer, status->m_keys, status->m_next_update,
                      status->m_expires);
  }
  status->m_done = true;
