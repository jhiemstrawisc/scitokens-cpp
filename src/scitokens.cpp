...
int scitoken_set_claim_string_list(const SciToken token, const char *key,
  ...
  if (real_token == nullptr) {
    if (err_msg)
      *err_msg =
          strdup("NULL scitoken passed to scitoken_get_claim_string_list");
    return -1;
  }
...
int scitoken_deserialize(const char *value, SciToken *token,
  ...
  int retval = scitoken_deserialize_v2(
      value, reinterpret_cast<SciToken>(real_token), allowed_issuers, err_msg);
  if (retval) {
    delete real_token;
  } else {
    *token = real_token;
...
void enforcer_acl_free(Acl *acls) {
  for (int idx = 0; acls[idx].authz == nullptr && acls[idx].resource == nullptr;
       idx++) {
...
Acl *convert_acls(scitokens::Enforcer::AclsList &acls_list, char **err_msg) {
  ...
  for (const auto &acl : acls_list) {
    ...
    if (acl_result[idx].authz == nullptr) {
      ...
      if (err_msg) {
        *err_msg = strdup("ACL was generated without an authorization set.");
      }
      return nullptr;
    }
...
int enforcer_generate_acls_continue(const Enforcer enf, SciTokenStatus *status,
  ...
  try {
    status_internal =
        real_enf->generate_acls_continue(std::move(status_internal), acls_list);
  } catch (std::exception &exc) {
    *status = nullptr;
    if (err_msg) {
      *err_msg = strdup(exc.what());
