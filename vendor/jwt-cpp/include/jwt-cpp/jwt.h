  ...
  public:
    ...
    std::string message(int ev) const override {
      ...
      case signature_verification_error::set_rsa_pss_saltlen_failed:
        return "failed to verify signature: EVP_PKEY_CTX_set_rsa_pss_saltlen "
               "failed";
  ...
  public:
    ...
    std::string message(int ev) const override {
      ...
      case signature_generation_error::rsa_padding_failed:
        return "failed to create signature: EVP_PKEY_CTX_set_rsa_padding "
               "failed";
      ...
      case signature_generation_error::set_rsa_pss_saltlen_failed:
        return "failed to create signature: EVP_PKEY_CTX_set_rsa_pss_saltlen "
               "failed";
...
struct ecdsa {
   ...
   * decrypt public key pem \param private_key_password Password to decrypt
   * private key pem \param md Pointer to hash function \param name Name of the
   * algorithm \param siglen The bit length of the signature
   */
  ecdsa(const std::string &public_key, const std::string &private_key,
    ...
    if (!private_key.empty()) {
      pkey = helper::load_private_ec_key_from_string(private_key,
                                                     private_key_password);
...
#else
    if (EVP_DigestSign(ctx.get(), reinterpret_cast<unsigned char *>(&res[0]),
                       &len,
                       reinterpret_cast<const unsigned char *>(data.data()),
                       data.size()) != 1) {
      ec = error::signature_generation_error::signfinal_failed;
...
template <typename traits> struct is_valid_traits {
  ...
  static_assert(supports_as_array<traits, typename traits::value_type,
...
struct is_valid_json_string {
  ...
  static_assert(substr,
                "string_type must have a substr method taking only a start "
...
public:
   ...
   * decode The function to decode the token \throw std::invalid_argument Token
   * is not in correct format \throw std::runtime_error Base64 decoding failed
   * or invalid json
   */
  template <typename Decode>
...
public:
   ...
   * signed input as a string_type \tparam Encode Callable method which takes a
   * string_type and base64url safe encodes it, MUST return the result with no
   * padding; trim the result. \param algo Instance of an algorithm to sign the
   * token with \param encode Callable to transform the serialized json to
...
template <typename json_traits> struct verify_context {
  ...
  // Current time, retrieved from the verifiers clock and cached for performance
  // and consistency
  date current_time;
...
public:
   ...
   * not present \throw std::bad_cast Claim was present but not a string (Should
   * not happen in a valid token)
   */
   ...
   */
  bool
  has_jwk_claim(const typename json_traits::string_type &name) const noexcept {
...
public:
  ...
  JWT_CLAIM_EXPLICIT jwks(const typename json_traits::string_type &str) {
    ...
    std::transform(
        jwk_list.begin(), jwk_list.end(), std::back_inserter(jwk_claims),
...
private:
  ...
  const_iterator
  find_by_kid(const typename json_traits::string_type &key_id) const noexcept {
