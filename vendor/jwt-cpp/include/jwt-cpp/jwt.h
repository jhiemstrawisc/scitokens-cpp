      ...
      public:
        ...
        std::string message(int ev) const override {
            ...
            case rsa_error::no_key_provided:
                return "at least one of public or private key need to be "
                       "present";
            default:
 ...
 * \param pw		Password used to decrypt certificate (leave empty if not
 * encrypted)
...
inline std::string extract_pubkey_from_cert(const std::string &certstr,
    ...
    std::unique_ptr<BIO, decltype(&BIO_free_all)> certbio(
        BIO_new_mem_buf(const_cast<char *>(certstr.data()),
 ...
 * \param certstr	String containing the certificate encoded as pem
 * \param pw		Password used to decrypt certificate (leave empty if not
 * encrypted) \throw			rsa_exception if an error occurred
 */
inline std::string extract_pubkey_from_cert(const std::string &certstr,
                                            const std::string &pw = "") {
    std::error_code ec;
...
std::string convert_base64_der_to_pem(const std::string &cert_base64_der_str,
    ...
    std::unique_ptr<BIO, decltype(&BIO_free_all)> certbio(BIO_new(BIO_s_mem()),
                                                          BIO_free_all);
    if (!cert || !certbio) {
        ec = error::rsa_error::create_mem_bio_failed;
        return {};
...
convert_base64_der_to_pem(const std::string &cert_base64_der_str,
    ...
    auto decode = [](const std::string &token) {
        return base::decode<alphabet::base64>(
            base::pad<alphabet::base64>(token));
    };
    return convert_base64_der_to_pem(cert_base64_der_str, std::move(decode),
                                     ec);
}
 ...
 * if not encrypted) \throw			rsa_exception if an error
 * occurred
 */
inline std::shared_ptr<EVP_PKEY>
 ...
 * occures)
 */
inline std::shared_ptr<EVP_PKEY> load_public_ec_key_from_string(
 ...
 * \param key		String containing the certificate or key encoded as pem
 * \param password	Password used to decrypt certificate or key (leave empty
 * if not encrypted) \throw			ecdsa_exception if an error
 * occurred
 */
inline std::shared_ptr<EVP_PKEY>
load_public_ec_key_from_string(const std::string &key,
                               const std::string &password = "") {
    std::error_code ec;
 ...
 * occures)
 */
inline std::shared_ptr<EVP_PKEY> load_private_ec_key_from_string(
    const std::string &key, const std::string &password, std::error_code &ec) {
    std::unique_ptr<BIO, decltype(&BIO_free_all)> privkey_bio(
        BIO_new(BIO_s_mem()), BIO_free_all);
    if (!privkey_bio) {
 ...
 * \param key		String containing a private key as pem
 * \param password	Password used to decrypt key (leave empty if not
 * encrypted) \throw			ecdsa_exception if an error occurred
 */
inline std::shared_ptr<EVP_PKEY>
load_private_ec_key_from_string(const std::string &key,
                                const std::string &password = "") {
    std::error_code ec;
...
#endif
{
    ...
    BN_bn2bin(
        bn, (unsigned char *)res.data()); // NOLINT(google-readability-casting)
                                          // requires `const_cast`
    return res;
}
/**
 ...
 */
inline std::unique_ptr<BIGNUM, decltype(&BN_free)>
...
struct none {
     ...
     */
    std::string sign(const std::string & /*unused*/,
...
struct hmacsha {
    ...
    std::string sign(const std::string &data, std::error_code &ec) const {
        ...
        if (HMAC(md(), secret.data(), static_cast<int>(secret.size()),
                 ...
                 (unsigned char *)
                     res.data(), // NOLINT(google-readability-casting)
                                 // requires `const_cast`
                 &len) == nullptr) {
...
struct rsa {
     ...
     * decrypt public key pem. \param private_key_password Password to decrypt
     * private key pem. \param md Pointer to hash function \param name Name of
     * the algorithm
     */
    rsa(const std::string &public_key, const std::string &private_key,
        ...
        if (!private_key.empty()) {
            ...
            pkey = helper::load_public_key_from_string(public_key,
                                                       public_key_password);
        } else
...
struct ecdsa {
     ...
     * decrypt public key pem \param private_key_password Password to decrypt
     * private key pem \param md Pointer to hash function \param name Name of
     * the algorithm \param siglen The bit length of the signature
     */
    ecdsa(const std::string &public_key, const std::string &private_key,
          const std::string &public_key_password,
          const std::string &private_key_password, const EVP_MD *(*md)(),
          std::string name, size_t siglen)
        ...
        if (!private_key.empty()) {
            ...
            pkey = helper::load_public_ec_key_from_string(public_key,
                                                          public_key_password);
    ...
    std::string sign(const std::string &data, std::error_code &ec) const {
        ...
        if (!EVP_DigestSignInit(ctx.get(), nullptr, md(), nullptr,
                                pkey.get())) {
            ec = error::signature_generation_error::signinit_failed;
            return {};
        }
        ...
        if (!EVP_DigestSignFinal(ctx.get(), (unsigned char *)res.data(),
                                 &len)) {
            ec = error::signature_generation_error::signfinal_failed;
            return {};
        }
    ...
    void verify(const std::string &data, const std::string &signature,
        ...
        if (!EVP_DigestVerifyInit(ctx.get(), nullptr, md(), nullptr,
                                  pkey.get())) {
            ec = error::signature_verification_error::verifyinit_failed;
            return;
        }
  ...
  private:
    ...
    std::string der_to_p1363_signature(const std::string &der_signature,
                                       std::error_code &ec) const {
        const unsigned char *possl_signature =
            reinterpret_cast<const unsigned char *>(der_signature.data());
        std::unique_ptr<ECDSA_SIG, decltype(&ECDSA_SIG_free)> sig(
            d2i_ECDSA_SIG(nullptr, &possl_signature, der_signature.length()),
            ECDSA_SIG_free);
        if (!sig) {
    ...
    std::string p1363_to_der_signature(const std::string &signature,
                                       std::error_code &ec) const {
        ec.clear();
...
struct eddsa {
     ...
     * empty, signing will always fail. \param public_key_password Password to
     * decrypt public key pem. \param private_key_password Password to decrypt
     * private key pem. \param name Name of the algorithm
     */
...
// EVP_DigestSignUpdate for eddsa, which is why we end up with this mess.
        ...
        if (EVP_DigestSignUpdate(
                ctx.get(), reinterpret_cast<const unsigned char *>(data.data()),
                data.size()) != 1) {
...
struct pss {
    ...
    std::string sign(const std::string &data, std::error_code &ec) const {
        ec.clear();
#ifdef JWT_OPENSSL_1_0_0
        std::unique_ptr<EVP_MD_CTX, decltype(&EVP_MD_CTX_destroy)> md_ctx(
            EVP_MD_CTX_create(), &EVP_MD_CTX_destroy);
#else
...
// exposes this functionality.
    ...
    void verify(const std::string &data, const std::string &signature,
        ...
        EVP_PKEY_CTX *ctx = nullptr;
        if (EVP_DigestVerifyInit(md_ctx.get(), &ctx, md(), nullptr,
                                 pkey.get()) != 1) {
            ec = error::signature_verification_error::verifyinit_failed;
            return;
        }
...
// exposes this functionality.
        ...
        if (EVP_PKEY_CTX_set_rsa_pss_saltlen(ctx, -1) <= 0) {
            ec =
                error::signature_verification_error::set_rsa_pss_saltlen_failed;
            return;
        }
#endif
        if (EVP_DigestUpdate(md_ctx.get(), data.data(), data.size()) != 1) {
        ...
        if (EVP_DigestVerifyFinal(md_ctx.get(),
...
struct es256k : public ecdsa {
     ...
     * private key pem.
     */
    explicit es256k(const std::string &public_key,
                    const std::string &private_key = "",
                    const std::string &public_key_password = "",
                    const std::string &private_key_password = "")
        : ecdsa(public_key, private_key, public_key_password,
                private_key_password, EVP_sha256, "ES256K", 64) {}
};
...
struct ed25519 : public eddsa {
     ...
     * \param private_key Ed25519 private key or empty string if not available.
     * If empty, signing will always fail. \param public_key_password Password
     * to decrypt public key pem. \param private_key_password Password to
     * decrypt private key pem.
     */
    explicit ed25519(const std::string &public_key,
                     const std::string &private_key = "",
                     const std::string &public_key_password = "",
                     const std::string &private_key_password = "")
        : eddsa(public_key, private_key, public_key_password,
                private_key_password, "EdDSA") {}
};
...
struct ed448 : public eddsa {
     ...
     * private key pem.
     */
    explicit ed448(const std::string &public_key,
                   const std::string &private_key = "",
                   const std::string &public_key_password = "",
                   const std::string &private_key_password = "")
        : eddsa(public_key, private_key, public_key_password,
...
using is_get_type_signature =
    typename std::is_same<get_type_function<traits_type>,
                          json::type(const value_type &)>;
...
using is_as_object_signature =
    typename std::is_same<as_object_function<traits_type>,
                          object_type(const value_type &)>;
...
using is_as_array_signature =
    typename std::is_same<as_array_function<traits_type>,
                          array_type(const value_type &)>;
...
using is_as_string_signature =
    typename std::is_same<as_string_function<traits_type>,
                          string_type(const value_type &)>;
...
using is_as_number_signature =
    typename std::is_same<as_number_function<traits_type>,
                          number_type(const value_type &)>;
...
using is_as_integer_signature =
    typename std::is_same<as_integer_function<traits_type>,
                          integer_type(const value_type &)>;
...
using is_as_boolean_signature =
    typename std::is_same<as_boolean_function<traits_type>,
                          boolean_type(const value_type &)>;
...
using is_begin_signature =
    typename std::is_same<decltype(std::declval<object_type>().begin()),
                          has_iterator<object_type>>;
...
using is_end_signature =
    typename std::is_same<decltype(std::declval<object_type>().end()),
                          has_iterator<object_type>>;
...
struct has_subcription_operator {
    ...
    static auto test_operator_plus(int) -> sfinae_true<
        decltype(std::declval<T>().operator[](std::declval<A0>()))>;
...
struct is_subcription_operator_signature {
    ...
    static_assert(
        ...
        "object_type must implementate the subscription operator '[]' "
        "for this library");
...
using is_at_const_signature =
    typename std::is_same<decltype(std::declval<const object_type>().at(
                              std::declval<const string_type>())),
                          const value_type &>;
...
struct is_valid_json_string {
    ...
    static_assert(
        substr, "string_type must have a substr method taking only a start "
                "index and an overload "
    ...
    static_assert(operator_plus,
                  "string_type must have a '+' operator implemented which "
...
template <typename value_type, typename string_type, typename integer_type,
          typename object_type, typename array_type>
struct is_valid_json_types {
    // Internal assertions for better feedback
    static_assert(
        is_valid_json_value<value_type>::value,
        "value_type must meet basic requirements, default constructor, "
        "copyable, moveable");
    static_assert(
...
template <typename json_traits> class basic_claim {
    ...
    static_assert(
        std::is_same<typename json_traits::string_type, std::string>::value ||
            std::is_convertible<typename json_traits::string_type,
                                std::string>::value ||
            std::is_constructible<typename json_traits::string_type,
                                  std::string>::value,
        "string_type must be a std::string, convertible to a std::string, or "
...
template <typename json_traits> class map_of_claims {
  ...
  public:
     ...
     */
    bool
    has_claim(const typename json_traits::string_type &name) const noexcept {
     ...
     * \throw jwt::error::claim_not_present_exception if the claim was not
     * present
     */
    basic_claim_t
    get_claim(const typename json_traits::string_type &name) const {
        if (!has_claim(name))
            throw error::claim_not_present_exception();
        return basic_claim_t{claims.at(name)};
    }

    std::unordered_map<typename json_traits::string_type, basic_claim_t>
    get_claims() const {
        static_assert(
            details::is_valid_json_object<
                typename json_traits::value_type,
                typename json_traits::string_type,
                typename json_traits::object_type>::supports_claims_transform,
            "currently there is a limitation on the internal implemantation of "
            "the "
        ...
        std::unordered_map<typename json_traits::string_type, basic_claim_t>
            res;
        std::transform(
            claims.begin(), claims.end(), std::inserter(res, res.end()),
...
template <typename json_traits> class payload {
  ...
  public:
     ...
     * \throw std::bad_cast Claim was present but not a string (Should not
     * happen in a valid token)
     */
     ...
     * \throw std::runtime_error If claim was not present
     * \throw std::bad_cast Claim was present but not a set (Should not happen
     * in a valid token)
     */
     ...
     * \throw std::runtime_error If claim was not present
     * \throw std::bad_cast Claim was present but not a date (Should not happen
     * in a valid token)
     */
     ...
     */
    basic_claim_t
    get_payload_claim(const typename json_traits::string_type &name) const {
...
template <typename json_traits> class header {
  ...
  public:
     ...
     */
    basic_claim_t
    get_header_claim(const typename json_traits::string_type &name) const {
...
class decoded_jwt : public header<json_traits>, public payload<json_traits> {
  ...
  public:
     ...
     */
    JWT_CLAIM_EXPLICIT
    decoded_jwt(const typename json_traits::string_type &token)
     ...
     * \brief Parses a given token
     *
     * \tparam Decode is callabled, taking a string_type and returns a
     * string_type. It should ensure the padding of the input and then base64url
     * decode and return the results. \param token The token to parse \param
     * decode The function to decode the token \throw std::invalid_argument
     * Token is not in correct format \throw std::runtime_error Base64 decoding
...
template <typename json_traits> class builder {
  ...
  public:
     ...
     * signed input as a string_type \tparam Encode Callable method which takes
     * a string_type and base64url safe encodes it, MUST return the result with
     * no padding; trim the result. \param algo Instance of an algorithm to sign
     * the token with \param encode Callable to transform the serialized json to
     * base64 with no padding \return Final token as a string
     *
     * \note If the 'alg' header in not set in the token it will be set to
     * `algo.name()`
     */
    template <typename Algo, typename Encode>
    typename json_traits::string_type sign(const Algo &algo,
                                           Encode encode) const {
        std::error_code ec;
     ...
     * no padding; trim the result. \param algo Instance of an algorithm to sign
     * the token with \param encode Callable to transform the serialized json to
     * base64 with no padding \param ec error_code filled with details on error
     * \return Final token as a string
     *
     * \note If the 'alg' header in not set in the token it will be set to
     * `algo.name()`
    ...
    typename json_traits::string_type sign(const Algo &algo, Encode encode,
        ...
        const auto header = encode(json_traits::serialize(
            typename json_traits::value_type(obj_header)));
        const auto payload = encode(json_traits::serialize(
...
template <typename json_traits> struct verify_context {
    ...
    basic_claim<json_traits> get_claim(bool in_header,
                                       std::error_code &ec) const {
        if (in_header) {
    ...
    basic_claim<json_traits> get_claim(bool in_header, json::type t,
                                       std::error_code &ec) const {
    ...
    basic_claim<json_traits> get_claim(json::type t,
                                       std::error_code &ec) const {
...
template <typename json_traits, bool in_header = false> struct equals_claim {
    ...
    void operator()(const verify_context<json_traits> &ctx,
                    std::error_code &ec) const {
...
struct insensitive_string_claim {
    ...
    std::locale locale;
    insensitive_string_claim(const typename json_traits::string_type &e,
                             std::locale loc)
    ...
    static std::string to_lower_unicode(const std::string &str,
...
template <typename Clock, typename json_traits> class verifier {
  ...
  private:
    ...
    template <typename T> struct algo : public algo_base {
        ...
        void verify(const std::string &data, const std::string &sig,
  ...
  public:
    ...
    explicit verifier(Clock c) : clock(c) {
        claims["exp"] = [](const verify_ops::verify_context<json_traits> &ctx,
            ...
            if (ctx.current_time >
                exp + std::chrono::seconds(ctx.default_leeway)) {
                ec = error::token_verification_error::token_expired;
            }
        };
        claims["iat"] = [](const verify_ops::verify_context<json_traits> &ctx,
            ...
            if (ctx.current_time <
                iat - std::chrono::seconds(ctx.default_leeway)) {
                ec = error::token_verification_error::token_expired;
            }
        };
        claims["nbf"] = [](const verify_ops::verify_context<json_traits> &ctx,
            ...
            if (ctx.current_time <
                nbf - std::chrono::seconds(ctx.default_leeway)) {
                ec = error::token_verification_error::token_expired;
            }
        };
    }
     ...
     * parameter is performed by the JWT application. Check is casesensitive.
     *
    ...
    verifier &with_type(const typename json_traits::string_type &type,
        ...
        return with_claim(
            "typ", verify_ops::insensitive_string_claim<json_traits, true>{
     ...
     * \return *this to allow chaining
     */
    verifier &with_claim(const typename json_traits::string_type &name,
     ...
     * \param ec error_code filled with details on error
     */
    void verify(const decoded_jwt<json_traits> &jwt,
                std::error_code &ec) const {
        ec.clear();
        const typename json_traits::string_type data =
        ...
        verify_ops::verify_context<json_traits> ctx{clock.now(), jwt,
                                                    default_leeway};
        for (auto &c : claims) {
...
template <typename json_traits> class jwk {
  ...
  public:
     ...
     * value. \return key type as string \throw std::runtime_error If claim was
     * not present \throw std::bad_cast Claim was present but not a string
     * (Should not happen in a valid token)
     */
     ...
     */
    basic_claim_t
    get_jwk_claim(const typename json_traits::string_type &name) const {
...
template <typename json_traits> class jwks {
  public:
    ...
    JWT_CLAIM_EXPLICIT jwks(const typename json_traits::string_type &str) {
        ...
        std::transform(jwk_list.begin(), jwk_list.end(),
     ...
     */
    bool
    has_jwk(const typename json_traits::string_type &key_id) const noexcept {
 ...
 */
template <typename json_traits> builder<json_traits> create() {
 ...
 */
template <typename json_traits>
decoded_jwt<json_traits>
decode(const typename json_traits::string_type &token) {
