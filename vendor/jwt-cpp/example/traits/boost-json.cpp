...
// #include <boost/json/src.hpp> // You may require this if you are not building
// it elsewhere
#include <iostream>
#include <sstream>
...
int main() {
  ...
  const auto token = jwt::create<traits>()
                         ...
                         .set_payload_claim("array", claim{big_numbers.begin(),
                                                           big_numbers.end()})
                         .set_payload_claim("object", from_raw_json)
                         .sign(jwt::algorithm::none{});
  const auto decoded = jwt::decode<traits>(token);

  const auto array = traits::as_array(decoded.get_payload_claim("object")
                                          .to_json()
                                          .as_object()["api"]
                                          .as_object()["array"]);
