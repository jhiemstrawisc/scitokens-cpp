...
#ifndef HUNTER_ENABLED // Static linking (which hunter always? does) breaks the
                       // tests (duplicate definition), so skip them
...
// LibreSSL has different return codes but was already outside of the effective
// scope
 ...
 * Once it is running and we want normal behaviour, we query the original method
 * using dlsym.
 */
...
X509 *PEM_read_bio_X509(BIO *bp, X509 **x, pem_password_cb *cb, void *u) {
    static X509 *(*origMethod)(BIO * bp, X509 * *x, pem_password_cb * cb,
                               void *u) = nullptr;
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_read_bio_X509");
...
int PEM_write_bio_PUBKEY(BIO *bp, OPENSSL_CONST EVP_PKEY *x) {
    ...
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_write_bio_PUBKEY");
...
int PEM_write_bio_X509(BIO *bp, OPENSSL_CONST X509 *x) {
    ...
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_write_bio_X509");
...
long BIO_ctrl(BIO *bp, int cmd, long larg, void *parg) {
    static long (*origMethod)(BIO * bp, int cmd, long larg, void *parg) =
        nullptr;
    if (origMethod == nullptr)
...
EVP_PKEY *PEM_read_bio_PUBKEY(BIO *bp, EVP_PKEY **x, pem_password_cb *cb,
                              void *u) {
    static EVP_PKEY *(*origMethod)(BIO * bp, EVP_PKEY * *x,
                                   pem_password_cb * cb, void *u) = nullptr;
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_read_bio_PUBKEY");
...
EVP_PKEY *PEM_read_bio_PrivateKey(BIO *bp, EVP_PKEY **x, pem_password_cb *cb,
    ...
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_read_bio_PrivateKey");
...
EC_KEY *PEM_read_bio_EC_PUBKEY(BIO *bp, EC_KEY **x, pem_password_cb *cb,
                               void *u) {
    static EC_KEY *(*origMethod)(BIO * bp, EC_KEY * *x, pem_password_cb * cb,
                                 void *u) = nullptr;
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_read_bio_EC_PUBKEY");
...
EC_KEY *PEM_read_bio_ECPrivateKey(BIO *bp, EC_KEY **x, pem_password_cb *cb,
                                  void *u) {
    static EC_KEY *(*origMethod)(BIO * bp, EC_KEY * *x, pem_password_cb * cb,
                                 void *u) = nullptr;
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "PEM_read_bio_ECPrivateKey");
...
unsigned char *HMAC(const EVP_MD *evp_md, const void *key, int key_len,
                    ...
                    unsigned int *md_len) {
    static unsigned char *(*origMethod)(const EVP_MD *evp_md, const void *key,
...
int EVP_DigestSignFinal(EVP_MD_CTX *ctx, unsigned char *sigret,
    ...
    static int (*origMethod)(EVP_MD_CTX * ctx, unsigned char *sigret,
                             size_t *siglen) = nullptr;
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_DigestSignFinal");
...
int EVP_DigestUpdate(EVP_MD_CTX *ctx, const void *d, size_t cnt) {
    static int (*origMethod)(EVP_MD_CTX * ctx, const void *d, size_t cnt) =
        nullptr;
    if (origMethod == nullptr)
...
int EVP_DigestFinal(EVP_MD_CTX *ctx, unsigned char *md, unsigned int *s) {
    static int (*origMethod)(EVP_MD_CTX * ctx, unsigned char *md,
...
int EVP_SignFinal(EVP_MD_CTX *ctx, unsigned char *md, unsigned int *s,
                  EVP_PKEY *pkey) {
    static int (*origMethod)(EVP_MD_CTX * ctx, unsigned char *md,
                             unsigned int *s, EVP_PKEY *pkey) = nullptr;
    if (origMethod == nullptr)
...
int EVP_VerifyFinal(EVP_MD_CTX *ctx, const unsigned char *sigbuf,
    ...
    static int (*origMethod)(EVP_MD_CTX * ctx, const unsigned char *sigbuf,
                             unsigned int siglen, EVP_PKEY *pkey) = nullptr;
    if (origMethod == nullptr)
...
int EVP_PKEY_public_check(EVP_PKEY_CTX *ctx) {
    ...
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_PKEY_public_check");
...
int EVP_PKEY_private_check(EVP_PKEY_CTX *ctx) {
    ...
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_PKEY_private_check");
...
EVP_PKEY_CTX *EVP_PKEY_CTX_new_from_pkey(OSSL_LIB_CTX *libctx, EVP_PKEY *pkey,
    ...
    static EVP_PKEY_CTX *(*origMethod)(OSSL_LIB_CTX * libctx, EVP_PKEY * pkey,
                                       const char *propquery) = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT,
...
EC_KEY *EVP_PKEY_get1_EC_KEY(EVP_PKEY *pkey) {
    ...
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_PKEY_get1_EC_KEY");
...
ECDSA_SIG *ECDSA_do_sign(const unsigned char *dgst, int dgst_len,
                         EC_KEY *eckey) {
    static ECDSA_SIG *(*origMethod)(const unsigned char *dgst, int dgst_len,
...
struct rsa_st *EVP_PKEY_get1_RSA(EVP_PKEY *pkey) {
    ...
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_PKEY_get1_RSA");
...
int EVP_DigestSignInit(EVP_MD_CTX *ctx, EVP_PKEY_CTX **pctx, const EVP_MD *type,
    ...
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_DigestSignInit");
...
int EVP_DigestVerifyInit(EVP_MD_CTX *ctx, EVP_PKEY_CTX **pctx,
    ...
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_DigestVerifyInit");
...
int EVP_DigestVerify(EVP_MD_CTX *ctx, unsigned char *sigret, size_t *siglen,
                     const unsigned char *tbs, size_t tbslen) {
    static int (*origMethod)(EVP_MD_CTX * ctx, unsigned char *sigret,
                             size_t *siglen, const unsigned char *tbs,
                             size_t tbslen) = nullptr;
...
int EVP_DigestVerifyFinal(EVP_MD_CTX *ctx, const unsigned char *sigret,
                          size_t siglen) {
    static int (*origMethod)(EVP_MD_CTX * ctx, const unsigned char *sigret,
                             size_t siglen) = nullptr;
    if (origMethod == nullptr)
        origMethod =
            (decltype(origMethod))dlsym(RTLD_NEXT, "EVP_DigestVerifyFinal");
...
int i2d_ECDSA_SIG(const ECDSA_SIG *sig, unsigned char **ppout) {
    static int (*origMethod)(const ECDSA_SIG *sig, unsigned char **ppout) =
        nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "i2d_ECDSA_SIG");
...
ECDSA_SIG *d2i_ECDSA_SIG(ECDSA_SIG **psig, const unsigned char **ppin,
                         long len) {
    static ECDSA_SIG *(*origMethod)(
        ECDSA_SIG * *psig, const unsigned char **ppin, long len) = nullptr;
    if (origMethod == nullptr)
        origMethod = (decltype(origMethod))dlsym(RTLD_NEXT, "d2i_ECDSA_SIG");
...
TEST(OpenSSLErrorTest, ConvertCertBase64DerToPemReference) {
    std::error_code ec;
    auto res =
        jwt::helper::convert_base64_der_to_pem(sample_cert_base64_der, ec);
...
TEST(OpenSSLErrorTest, LoadPublicKeyFromStringErrorCode) {
    ...
    run_multitest(mapping, [](std::error_code &ec) {
        auto res =
            jwt::helper::load_public_key_from_string(rsa_pub_key, "", ec);
...
TEST(OpenSSLErrorTest, LoadPublicKeyCertFromStringErrorCode) {
    ...
    run_multitest(mapping, [](std::error_code &ec) {
        auto res =
            jwt::helper::load_public_key_from_string(sample_cert, "", ec);
        ASSERT_EQ(res, nullptr);
    });
}
...
TEST(OpenSSLErrorTest, LoadPrivateKeyFromString) {
    std::vector<multitest_entry> mapping{
        {&fail_BIO_new, 1, jwt::error::rsa_error::create_mem_bio_failed},
        {&fail_BIO_write, 1, jwt::error::rsa_error::load_key_bio_write},
        {&fail_PEM_read_bio_PrivateKey, 1,
         jwt::error::rsa_error::load_key_bio_read}};
...
TEST(OpenSSLErrorTest, LoadPrivateKeyFromStringErrorCode) {
    ...
    run_multitest(mapping, [](std::error_code &ec) {
        auto res =
            jwt::helper::load_private_key_from_string(rsa_priv_key, "", ec);
        ASSERT_EQ(res, nullptr);
    });
}
...
TEST(OpenSSLErrorTest, RS256Reference) {
    ...
    ASSERT_EQ(
        jwt::base::encode<jwt::alphabet::base64>(res),
        "oCJUeLmIKKVVE/UWhEL/"
        "Malx0l9TCoXWNAS2z9o8ZYNaS4POIeadZWeUbLdICx3SCJCnGRwL8JkAmYx1wexT2Q"
        "GuVXXAtZvRu8ceyuQy"
        "AhzGkI9HdADu5YAJsUaLknDUV5hmundXQY8lhwQnKFXW0rl0H8DoFiPQErFmcKI6PA"
        "9NVGK/LSiqHqesNeg0wqCTxMmeT6pqI7FH9fDO"
        "CaBpwUJ4t5aKoytQ75t13OfUM7tfLlVkFZtI3RndhivxLA5d4Elt/"
        "Gv3RhDu6Eiom5NZ/pwRvP26Sox+FWapz3DGCil70H1iGSYu8ENa"
...
TEST(OpenSSLErrorTest, RS256VerifyErrorCode) {
    ...
    auto signature = jwt::base::decode<jwt::alphabet::base64>(
        "oCJUeLmIKKVVE/UWhEL/"
        "Malx0l9TCoXWNAS2z9o8ZYNaS4POIeadZWeUbLdICx3SCJCnGRwL8JkAmYx1wexT2QGuVX"
        "XA"
        "tZvRu8ceyuQy"
        "AhzGkI9HdADu5YAJsUaLknDUV5hmundXQY8lhwQnKFXW0rl0H8DoFiPQErFmcKI6PA9NVG"
        "K/"
        "LSiqHqesNeg0wqCTxMmeT6pqI7FH9fDO"
        "CaBpwUJ4t5aKoytQ75t13OfUM7tfLlVkFZtI3RndhivxLA5d4Elt/Gv3RhDu6Eiom5NZ/"
        "pwRvP26Sox+FWapz3DGCil70H1iGSYu8ENa"
        "afUBCGGhT4sk7kl7zS6XiEpMobLq3A==");
    std::vector<multitest_entry> mapping{
        {&fail_EVP_MD_CTX_new, 1,
...
TEST(OpenSSLErrorTest, LoadECDSAPrivateKeyFromString) {
    std::vector<multitest_entry> mapping{
        ...
        {&fail_EVP_PKEY_CTX_new_from_pkey, 1,
...
TEST(OpenSSLErrorTest, ES256SignErrorCode) {
    ...
    std::vector<multitest_entry> mapping{
        {&fail_EVP_MD_CTX_new, 1,
         jwt::error::signature_generation_error::create_context_failed},
        {&fail_EVP_DigestSignInit, 1,
         jwt::error::signature_generation_error::signinit_failed},
        {&fail_EVP_DigestUpdate, 1,
         jwt::error::signature_generation_error::digestupdate_failed},
        {&fail_EVP_DigestSignFinal, 1,
         jwt::error::signature_generation_error::signfinal_failed},
        {&fail_EVP_DigestSignFinal, 2,
         jwt::error::signature_generation_error::signfinal_failed},
        {&fail_d2i_ECDSA_SIG, 1,
...
TEST(OpenSSLErrorTest, ES256VerifyErrorCode) {
    jwt::algorithm::es256 alg{ecdsa256_pub_key, ecdsa256_priv_key};
    auto signature = jwt::base::decode<jwt::alphabet::base64>(
        "aC/NqyHfPw5FDA0yRAnrbkrAlXjsr0obRkCg/"
        "HgP+77QYJrAg6YKkKoJwMXjUX8fQrxXKTN7em5L9dtmOep37Q==");
    std::vector<multitest_entry> mapping{
        {&fail_EVP_MD_CTX_new, 1,
         jwt::error::signature_verification_error::create_context_failed},
        {&fail_EVP_DigestVerifyInit, 1,
         jwt::error::signature_verification_error::verifyinit_failed},
        {&fail_EVP_DigestUpdate, 1,
         jwt::error::signature_verification_error::verifyupdate_failed},
        {&fail_EVP_DigestVerifyFinal, 1,
         jwt::error::signature_verification_error::invalid_signature},
        {&fail_ECDSA_SIG_new, 1,
         jwt::error::signature_verification_error::create_context_failed},
        {&fail_i2d_ECDSA_SIG, 1,
         jwt::error::signature_verification_error::signature_encoding_failed},
        {&fail_i2d_ECDSA_SIG, 2,
         jwt::error::signature_verification_error::signature_encoding_failed},
    };
...
TEST(OpenSSLErrorTest, PS256VerifyErrorCode) {
    ...
    std::string signature =
        "LMiWCiW0a/"
        "mbU6LK8EZaDQ6TGisqfD+LF46zUbzjhFt02J9yVuf3ZDNTdRgLKKP8nCJUx0SN+"
        "5CS2YD268Ioxau5bWs49RVCxtID5DcRpJlSo+Vk+"
        "dCmwxhQWHX8HNh3o7kBK5H8fLeTeupuSov+0hH3+"
        "GRrYJqZvCdbcadi6amNKCfeIl6a5mp2VCM55NsPoRxsmSzc1G7AHWb1ckOCsm3KY5BL6B0"
        "74"
        "bHgoqO3yaLlKWLAcy4OYyRpJ/wnZQ9PPrhwdq/"
        "B59uW3x1QUCKYKgZeqZOoqIP1YgLwvEpPtXYutQCFr4eBKgV7vdtE0wgHR43ka16fi5L4S"
        "ya"
        "Zv53NCg==";
