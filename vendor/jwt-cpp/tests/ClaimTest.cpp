...
TEST(ClaimTest, SetAudienceAsString) {
  auto token = jwt::create().set_type("JWT").set_audience("test").sign(
      jwt::algorithm::hs256("test"));
  ASSERT_EQ("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJ0ZXN0In0."
            "ny5Fa0vzAg7tNL95KWg_ecBNd3XP3tdAzq0SFA6diY4",
            token);
}

TEST(ClaimTest, AudienceAsSet) {
  std::string token =
      "eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJhdWQiOlsidGVzdCIsInRlc3QyIl19.";
  auto decoded = jwt::decode(token);
...
TEST(ClaimTest, SetObject) {
  ...
  ASSERT_EQ(token, "eyJhbGciOiJIUzI1NiJ9.eyJuYW1lc3BhY2UiOnsiYXBpLXgiOlsxXX19."
                   "F8I6I2RcSF98bKa0IpIz09fRZtHr1CWnWKx2za-tFQA");
}
...
TEST(ClaimTest, PicoJSONTraitsAsBool) {
  ...
  ASSERT_EQ(jwt::traits::kazuho_picojson::get_type(val),
...
TEST(ClaimTest, MapOfClaim) {
  ...
  ASSERT_THROW(map::parse_claims(R"##(__ not json __)##"),
               jwt::error::invalid_json_exception);
  const map claims{map::parse_claims(
      R"##({ "array": [1], "string" : "hello world", "number": 9.9, "bool": true})##")};
  ...
  ASSERT_EQ(
      map::basic_claim_t{claims.get_claim("array").as_array().at(0)}.as_int(),
      (int)1);
  ...
  ASSERT_THROW(claims.get_claim("__missing__"),
