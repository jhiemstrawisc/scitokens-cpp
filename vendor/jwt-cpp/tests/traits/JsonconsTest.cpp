...
TEST(JsonconsTest, BasicClaims) {
  ...
  const auto array = jwt::basic_claim<jwt::traits::danielaparker_jsoncons>(
      std::set<jwt::traits::danielaparker_jsoncons::string_type>{"string",
                                                                 "string"});
  ASSERT_EQ(array.get_type(), jwt::json::type::array);

  const auto integer =
      jwt::basic_claim<jwt::traits::danielaparker_jsoncons>(159816816);
...
TEST(JsonconsTest, SetArray) {
  ...
  auto token =
      jwt::create<jwt::traits::danielaparker_jsoncons>()
          .set_payload_claim(
              "test", jwt::basic_claim<jwt::traits::danielaparker_jsoncons>(
                          vect.begin(), vect.end()))
...
TEST(JsonconsTest, VerifyTokenHS256) {
  ...
  const auto decoded_token =
      jwt::decode<jwt::traits::danielaparker_jsoncons>(token);
...
TEST(JsonconsTest, VerifyArray) {
  ...
  std::vector<int64_t> vect = {100, 20, 10};
  jwt::basic_claim<jwt::traits::danielaparker_jsoncons> array_claim(
      vect.begin(), vect.end());
  const auto verify = jwt::verify<jwt::traits::danielaparker_jsoncons>()
